...
def get_fasttext():
    fasttext_model_path = "/scratch/project_2001083/sanchit/models/crawl-300d-2M-subword.bin"
    print('load fasttext')
    model = fasttext.load_model(fasttext_model_path)
    get_dimension = model.get_dimension()
    
    return model

class LightXML(nn.Module):
    def __init__(self, n_labels, group_y=None, bert='fasttext', feature_layers=5, dropout=0.5, update_count=1,
                 candidates_topk=10, 
        ...
        self.bert_name, self.bert = fasttext, get_fasttext()
        ...
        if self.group_y is not None:
            ...
            #Label Recalling
            self.l0 = nn.Linear(self.feature_layers*self.bert.get_dimension(), self.group_y_labels)
            # hidden bottle layer
            self.l1 = nn.Linear(self.feature_layers*self.bert.get_dimension(), hidden_dim)
            self.embed = nn.Embedding(n_labels, hidden_dim)
            nn.init.xavier_uniform_(self.embed.weight)
        else:
            self.l0 = nn.Linear(self.feature_layers*self.bert.get_dimension(), n_labels)
    ...
    def get_fast_tokenizer(self):
        print('load fasttext tokenizer')
        tokenizer = fasttext.load_model("/scratch/project_2001083/sanchit/models/crawl-300d-2M-subword.bin")
        return tokenizer
