...
def createDataCSV(dataset):
    ...
    name_map = {'wiki31k': 'Wiki10-31K',
                ...
                'eurlex4k': 'Eurlex-4K',
                'amazontitles300k':'AmazonTitles-300K'}

    assert dataset in name_map
    dataset = name_map[dataset]

    fext = '_texts.txt' if dataset == 'Eurlex-4K' else '_raw_texts.txt'
    with open(f'/scratch/project_2001083/nasib/Datasets/{dataset}/train{fext}') as f:
        for i in tqdm.tqdm(f):
            texts.append(i.replace('\n', ''))
            dataType.append('train')

    with open(f'/scratch/project_2001083/nasib/Datasets/{dataset}/test{fext}') as f:
        for i in tqdm.tqdm(f):
            texts.append(i.replace('\n', ''))
            dataType.append('test')

    with open(f'/scratch/project_2001083/nasib/Datasets/{dataset}/train_labels.txt') as f:
        for i in tqdm.tqdm(f):
            for l in i.replace('\n', '').split():
                label_map[l] = 0
    ...
    with open(f'/scratch/project_2001083/nasib/Datasets/{dataset}/test_labels.txt') as f:
        print(len(label_map))
        for i in tqdm.tqdm(f):
            for l in i.replace('\n', '').split():
                label_map[l] = 0
...
class MDataset(Dataset):
    def __init__(self, df, mode, tokenizer, label_map, max_length,
        ...
        self.tokenize, self.max_length, self.group_y = tokenizer, max_length, group_y
    ...
    def __getitem__(self, idx):
        ...
        labels = [self.label_map[i] for i in self.df.label.values[idx].split() if i in self.label_map] #if condition to handle missing labels in train.
        ...
        elif hasattr(self.tokenize, 'encode_plus'):
        ...
        if self.group_y is not None:
            ...
            if self.mode == 'train':
                return input_ids, attention_mask, token_type_ids,label_ids[candidates], group_label_ids, candidates
            else:
                return input_ids, attention_mask, token_type_ids,label_ids, group_label_ids, candidates
            # root fc layer
